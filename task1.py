# -*- coding: utf-8 -*-
"""ananditasDocumentation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tu0dpS8aUHkevuU66jicghVOCuhbgrxP
"""
...
#creates adjacency matrix (edges)
def create_adj_matrix(n_chans):
    adj = torch.ones(n_chans, n_chans)#129x129, each elem is 1
    adj.fill_diagonal_(0)#no self loops
    return adj

#Pretraining using encoder-decoder
#Encoder code
#takes raw eeg window data, does temporal convolutions and uses gcn for latent repres
class GCNEncoder(nn.Module):
  #inout is 129x200 converted to 129x32
    def __init__(self, chans=129, samples=200, gcn_out_dim=32):
        #initalises, we want 32 dimension coming out
        super().__init__()
        self.adj = create_adj_matrix(chans)
        self.gcn_out_dim = gcn_out_dim
        self.chans = chans

#we doing temporal using cnn and spatial using gcn
       #temporal features extracting here
        self.temp_features = nn.Sequential(
            nn.Conv2d(1, 16, (1, 5), padding=(0, 2)),
            nn.BatchNorm2d(16), #(1,5)5 samples wide, only covers one feature at a time
            nn.ELU(),

            nn.Conv2d(16, 32, (1, 5), padding=(0, 2)),
            nn.BatchNorm2d(32),
            nn.ELU(),
            nn.AvgPool2d((1, 2)), #downsampled by half
        )

        temporal_feature_size = 32 * (samples // 2)

        #aggregates features of neighbouring channels
        self.gcn_layer = GCNConv(temporal_feature_size, gcn_out_dim)
        self.out_dim = chans * gcn_out_dim #129x32
#cnn reshapes, gcn flattens
    def forward(self, x):
        B, C, T = x.shape


        if not hasattr(self, 'edge_index') or self.edge_index.device != x.device:
            self.edge_index = dense_to_sparse(self.adj)[0].to(x.device)
#makes edge index matrix pytorch geometric compatible, makes it into a sparse list of connected pairs

        x_conv = x.unsqueeze(1)
        x_temp_features = self.temp_features(x_conv)


        x_gcn_input = x_temp_features.permute(0, 2, 1, 3).contiguous()
        x_gcn_input = x_gcn_input.view(B, C, -1)

        gcn_output = []
        for i in range(B):
            node_features = x_gcn_input[i]
            z_i = self.gcn_layer(node_features, self.edge_index)
            gcn_output.append(z_i)

        z = torch.stack(gcn_output, dim=0)

        z = z.flatten(start_dim=1)
        return z

class GCNColumnDecoder(nn.Module):
...
class GCNClassifier(nn.Module):
    def __init__(self, encoder: GCNEncoder, num_classes=1):
        super().__init__()
...
PRETRAINED_WEIGHTS_PATH = "encoder_passive_maskedSSL_GCN.pt"

#pretraining
#BIDS is brain imaging data structure, we find finds which have passive task data
#BDF is biosemi data format
...
encoder = GCNEncoder().to(device) # my model
decoder = GCNColumnDecoder(encoder.out_dim).to(device) #my model

#masked signal modelling in ssl
if BIDS_ROOT is not None and len(PASSIVE_BDFS) > 0:
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"average SSL loss: {avg_epoch_loss:.6f}")

    # Save pretrained encoder
    torch.save(encoder.state_dict(), PRETRAINED_WEIGHTS_PATH)
    print(f"pretrained encoder to {PRETRAINED_WEIGHTS_PATH}")
...
ANCHOR = "stimulus_anchor" #starting point
SHIFT_AFTER_STIM = 0.5 #offset
WINDOW_LEN = 2.0
...
pretrained_gcn_encoder = GCNEncoder().to(device)
model = GCNClassifier(encoder=pretrained_gcn_encoder).to(device)
...
try:
  ...
  #so that i know encoder weights
    mapped_state_dict = {}
    for k, v in pretrained_state_dict.items():
        mapped_state_dict[f"encoder.{k}"] = v

    classifier_state_dict.update(mapped_state_dict)
    model.load_state_dict(classifier_state_dict)
#overwrite randomly intialised weights
...
torch.save(model.state_dict(), "weights_challenge_1_GCN_transfer.pt")
print("Model saved as 'weights_challenge_1_GCN_transfer.pt'")